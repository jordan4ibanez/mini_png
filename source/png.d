...
void convertPngData(ubyte type, ubyte depth, const(ubyte)[] data, int width, ubyte[] idata, ref size_t idataIdx) {
    ...
    loop: for (int pixel = 0; pixel < width; pixel++) switch (type) {
    case 0: // greyscale
    case 4: // greyscale with alpha
    case 3: // indexed
    ...
    case 6: // true with alpha
        if (depth == 8) {
            ...
            assert(0, "unsupported truecolor bit depth " ~ to!string(depth));
        break;
    default:
        assert(0);
    }
...
PNG* pngFromImage(IndexedImage i) {
    ...
    if (i.numColors() <= 2)
        ...
        throw new Exception("can't save this as an indexed png");
    ...
    bool insertChunk(Chunk* chk, bool replaceExisting = false) {
        ...
        foreach_reverse (immutable idx, ref cc; chunks) {
            ...
            if (cc.stype == "IDAT") {
                ...
                chunks.length += 1;
                foreach_reverse (immutable c; idx + 1 .. chunks.length)
                    chunks.ptr[c] = chunks.ptr[c - 1];
                chunks.ptr[idx] = *chk;
                return false;
            }
        }
...
ubyte[] getANDMask(PNG* p) {
    ...
    for (int a = 0; a < h.height; a++) {
        ...
        for (int b = 0; b < h.width; b++) {
            if (h.depth == 4) {
                ufdata[pos / 8] |= ((colors[bits ? getLowNybble(data[pos2]): getHighNybble(
                        data[pos2])].a <= 30) << (7 - (pos % 8)));
            } else
...
uint update_crc(in uint crc, in ubyte[] buf) {
    static const uint[256] crc_table = [
...
/// See: [readPngChunks]
struct LazyPngChunks(T) if (isBufferedInputRange!(T) && is(ElementType!T == ubyte[])) {
    T bytes;
    Chunk current;
...
struct LazyPngFile(LazyPngChunksProvider)
    ...
    auto rawDatastreamByChunk(int chunkSize = 0) {
        ...
        struct DatastreamByChunk(T) {
            ...
            ~this() {
                version (arsdpng_debug) {
                    import core.stdc.stdio : printf;

                    printf("destroying lazy PNG reader...\n");
                }
            ...
            ubyte[] front() {
                return (bufpos > 0 ? buffer[0 .. bufpos] : null);
            }
                ...
                while (plpos != plpos.max && bufpos < chunkSize) {
                    ...
                    if (zs.avail_in > 0) {
                        ...
                        if (err != Z_STREAM_END && err != Z_OK)
                            throw new Exception("PNG unpack error");
                        if (err == Z_STREAM_END) {
                            if (zs.avail_in != 0) {
                    ...
                    if (plpos == plpos.max || plpos >= chunks.front.payload.length) {
                        ...
                        if (chunks.front.stype != "IDAT")
                            break; // this chunk is not IDAT, that means that... alas
                        chunks.popFront(); // remove current IDAT
                        plpos = 0;
                        if (chunks.empty || chunks.front.stype != "IDAT")
                            plpos = plpos.max; // special value
                        continue;
                    }
                    if (plpos < chunks.front.payload.length) {
                        // current chunk is not complete, get some more bytes from it
                        int rd = cast(int)(chunks.front.payload.length - plpos <= pkbuf.length ? chunks
                                .front.payload.length - plpos : pkbuf.length);
                        assert(rd > 0);
    ...
    version (none) auto byRgbaScanline() {
    ...
    case 3:
        ...
        if (previousLine.length)
            foreach (i; 0 .. arr.length) {
                ...
                arr[i] += cast(ubyte) /*std.math.floor*/
                    (cast(int)(prev + (previousLine.length ? previousLine[i] : 0)) / 2);
            }
