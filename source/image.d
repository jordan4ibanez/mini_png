...
public ImageFileFormat guessImageFormatFromExtension(const(char)[] filename) {
    ...
    if (strEquCI(ext, "jpg") || strEquCI(ext, "jpeg"))
...
public ImageFileFormat guessImageFormatFromMemory(const(void)[] membuf) {
    ...
    if (buf.length > 6 && buf.ptr[0] == 'B' && buf.ptr[1] == 'M') {
        ...
        if (datasize > 6 && datasize <= buf.length)
            return ImageFileFormat.Bmp;
    }
    // gif
    ...
    try {
        ...
        if (detect_jpeg_image_from_memory(buf, width, height, components))
            return ImageFileFormat.Jpeg;
    } catch (Exception e) {
    } // sorry
    ...
    bool guessTarga() nothrow @trusted @nogc {
        ...
        if (wImageWidth < 1 || wImageHeight < 1 || wImageWidth > 32_000 || wImageHeight > 32_000)
        ...
        case 0:
            if (wColorMapFirstEntryIndex != 0 || wColorMapLength != 0)
                return 0;
            break;
        case 1:
            if (bColorMapEntrySize != 15 && bColorMapEntrySize != 16 && bColorMapEntrySize != 24 &&
                bColorMapEntrySize != 32)
                return false;
    ...
    bool guessPcx() nothrow @trusted @nogc {
        if (buf.length < 129)
            return false; // we should have at least header
    ...
    static if (is(T == typeof(null))) {
        ...
        case ImageFileFormat.Bmp:
            static if (is(T == string))
                ...
                return readBmp(filename.idup);
        case ImageFileFormat.Jpeg:
            return readJpeg(filename);
        case ImageFileFormat.Gif:
            throw new Exception("arsd has no GIF loader yet");
        case ImageFileFormat.Tga:
            return loadTga(filename);
        case ImageFileFormat.Pcx:
            return loadPcx(filename);
        case ImageFileFormat.Svg:
            static if (is(T == string))
                return readSvg(filename);
            else
                return readSvg(filename.idup);
...
public MemoryImage loadImageFromMemory(const(void)[] membuf) {
    ...
    case ImageFileFormat.Bmp:
        return readBmp(cast(const(ubyte)[]) membuf);
    case ImageFileFormat.Jpeg:
        return readJpegFromMemory(cast(const(ubyte)[]) membuf);
    case ImageFileFormat.Gif:
        throw new Exception("arsd has no GIF loader yet");
    case ImageFileFormat.Tga:
        return loadTgaMem(membuf);
    case ImageFileFormat.Pcx:
        return loadPcxMem(membuf);
    case ImageFileFormat.Svg:
        return readSvg(cast(const(ubyte)[]) membuf);
    case ImageFileFormat.Dds:
        return ddsLoadFromMemory(membuf);
    }
}
